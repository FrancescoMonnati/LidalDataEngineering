...
def process_directory(directory, matlab_queue, year,d0,server,database, username,password,table,release, chaos = True, prediction_df = None, clearing = False,process_start_time=None, total_files=0, completed_files=0):
...
class TimeEstimator:
    ...
    def update_progress(self, task_time=None):
    ...
    def get_time_estimate(self):
        ...
        return elapsed_time, estimated_remaining_time, avg_time_per_task
    
    def log_progress(self, logger, current_task_name=""):
        elapsed, remaining, avg_time = self.get_time_estimate()
        if elapsed is not None:
            logger.info(f"Progress: {self.completed_tasks}/{self.total_tasks} files processed")
            logger.info(f"Current task: {current_task_name}")
            logger.info(f"Elapsed time: {elapsed/60:.1f} min | Estimated remaining: {remaining/60:.1f} min")
